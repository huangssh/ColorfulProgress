...
public class UtilUnitConversion {
	 ...
	 * @return String	合适单位值 如3.5G, 3.5M, 3.5K
	 */
	public static String getFlowFromByteMathFloor(double flowByte, int decimal){
		String strFlow = null;
		String ceils[] = {"K","M","G","T"};
		int i=0;
		for(;flowByte>=1024 && i<ceils.length-1;i++){
		...
		flowByte = Math.floor(flowByte * 100) / 100.0;
	...
	/**
	 * 流量单位转换并向下取整,在流量小于1024的时候也用M为单位
	 *
	 * @author jiangwenxin
	 * @createTime 2014年9月15日 下午4:32:40
	 * @param flowByte 流量值 单位KB
	 ...
	 * @return String	合适单位值 如3.5G, 3.5M, 3.5K
	 */
	public static String getSimpleFlowFromByteMathFloor(double flowByte, int decimal){
		String strFlow = null;
		String ceils[] = {"M","M","G","T"};
		int i=0;
		if (flowByte >= 1024){
			for(;flowByte>=1024 && i<ceils.length-1;i++){
				flowByte = flowByte/1024.0;
     ...
     * @return
     */
	public static float calculationIntegerResults(float fee, String t){
		//截取小数点后两位小数，解决因float精度原因导致强制类型转换时会出现误差的问题（例如985.0实际fee是985.00006）
		float result = (int)(fee * 100) / 100.0f;
		int type = 0;
		if (t != null && !"".equals(t)){
